...
api.mapkey(';rc', "enable right click",function(){
  var d=document;
  var s=['user-select','-webkit-user-select','-moz-user-select','-ms-user-select'];
  var a=['mousedown','selectstart','copy','contextmenu'];
  var p=function(e){e.stopPropagation();};
  ['mousedown','contextmenu','selectstart','copy','keydown','keypress','keyup'].forEach(
    function(n){
  ...
  Array.prototype.forEach.call(document.querySelectorAll('*'), function(l){
    s.forEach(function(t){l.style.setProperty(t,'auto','important');});
    a.forEach(function(n){l.addEventListener(n,p,true);})
  ;})
;})
...
// DMM 単品CDレンタル
api.addSearchAlias('C', 'DMM 単品CDレンタル', 'https://www.dmm.com/rental/ppr/-/search/=/searchstr={0}/floor=cd/limit=120/');
...
// Chrome ウェブストア
api.addSearchAlias('cw', 'chrome ウェブストア', 'https://chrome.google.com/webstore/search?q=');
api.mapkey('ocw', 'Search with alias chrome ウェブストア', function() {
  api.Front.openOmnibar({ type: "SearchEngine", extra: "cw" });
});
// DMM 単品DVDレンタル
api.addSearchAlias('D', 'DMM 単品DVDレンタル', 'https://www.dmm.com/rental/ppr/-/search/=/searchstr={0}/floor=dvd/limit=120/');
...
api.addSearchAlias('E', 'Google 日本除外 1年以内', 'https://www.google.com/search?q={0}&gl=us&hl=en&pws=0&tbs=qdr:y-lang_ja');
...
// Gist my
api.addSearchAlias('i', 'Gist my', 'https://gist.github.com/search?q=user%3Amasato3+{0}&ref=searchresults');
api.mapkey('oi', 'Search with alias Gist my', function() {
  api.Front.openOmnibar({ type: "SearchEngine", extra: "i" });
});
// Gist
api.addSearchAlias('I', 'Gist', 'https://gist.github.com/search?utf8=%E2%9C%93&q=');
...
api.addSearchAlias('J', 'JVN iPedia', 'https://jvndb.jvn.jp/search/index.php?mode=_vulnerability_search_IA_VulnSearch&lang=ja&useSynonym=1&keyword=');
...
api.addSearchAlias('ms', 'まとも検索', 'https://www.google.com/search?q=site%3Aac.jp+OR+site%3Ago.jp+OR+site%3Alg.jp+');
...
api.addSearchAlias('R', 'Yahoo!リアルタイム検索 画像/動画', 'https://search.yahoo.co.jp/realtime/search?p={0}&ei=UTF-8&mtype=image');
api.mapkey('oR', 'Search with alias Yahoo!リアルタイム検索 画像/動画', function() {
  api.Front.openOmnibar({ type: "SearchEngine", extra: "R" });
});
// Twitter
...
// Twitter user 最新
api.addSearchAlias('tu', 'twitter user 最新', 'https://twitter.com/search?q=from:{0}&f=live');
api.mapkey('otu', 'Search with alias twitter user 最新', function() {
  api.Front.openOmnibar({ type: "SearchEngine", extra: "tu" });
});
// Twitter my 最新
api.addSearchAlias('tm', 'twitter my 最新', 'https://twitter.com/search?q=from:masatonyoron%20{0}&f=live');
...
api.unmapAllExcept(
  [
    'p', 'gi', 'I', 'f', 'q', 'F', '[', ']', 'gg', 'G', '<Ctrl-k>', '<Ctrl-j>', 'yt', 'gxa', 'gxe', 'ga', 'ge', 'gp', 'T', 'on', 'x', 'X',
    'W', 'h', 'l', '<', '>', 'r', 'L', 'H', 'sG', 'se', 'sj', 'spd', 'sw', 'yy', 'yn', 't', 'o1', 'o2', 'o3', 'o6', 'ox', 'oh', 'og', 'oy',
    'oM', 'oa', 'oA','oC', 'oci', 'ocl', 'ocw', 'od', 'oD', 'oe', 'oE', 'of', 'oF', 'oG', 'oi', 'oI', 'oj', 'ok', 'omb', 'oms', 'omw',
    'oo', 'opa', 'opd', 'ops', 'oq', 'or', 'ota', 'ott', 'otu', 'otm', 'ow', 'oz', 'o!', 'v', 'b', 'gn', 'go', "'", ';e', 'gE', 'gH', 'gK',
    'gS', 'gD'
  ],
  /www.pixiv.net/
);
...
api.unmapAllExcept(
  [
    'p', 'i', 'I', 'f', 'q', 'F', 'gg', 'G', 'j', 'k', '<Ctrl-k>', '<Ctrl-j>', 'yt', 'ga', 'ge', 'gp', 'T', 'on', 'X', 'W', 'h', 'l', '<',
    '>', 'L', 'H', 'yy', 't', 'b', 'gn', 'go', "'", 'ymd'
  ],
  /toggl.com/
);
...
api.unmapAllExcept(
  [
    'p', 'f', 'I', 'F', 'mf', 'gg', 'G', 'j', 'k', '<Ctrl-k>', '<Ctrl-j>', 'yt', 'gxe', 'ga', 'ge', 'gp', 'T', 'on', 'X', 'W', 'h', 'l',
    '<', '>', 'L', 'H', 'sG', 'yy', 't', 'o1', 'o2', 'o3', 'o6', 'ox', 'oh', 'og', 'oy', 'oM', 'oa', 'oa', 'oC', 'oci', 'ocl', 'ocw', 'od',
    'oD', 'oe', 'oE', 'of', 'oF', 'oG', 'oi', 'oI', 'oj', 'oJ', 'ok', 'omb', 'oms', 'omw', 'oo', 'opa', 'opd', 'ops','oq', 'or', 'ota',
    'ott', 'otu', 'otm', 'ots', 'ow', 'oz', 'o!', 'b', 'gn', 'go', "'", ';e', 'gE', 'gH', 'gK', 'gS', 'gD', 'gwt'
  ],
  /www.toodledo.com/
);
...
api.unmapAllExcept(
  [
    'p', 'gi', 'I', 'f', 'F', 'mf', 'gg', 'G', '<Ctrl-k>', '<Ctrl-j>', 'yt', 'gxe', 'ga', 'ge', 'gp', 'T', 'on', 'X', 'W', 'h', 'l', '<',
    '>', 'L', 'H', 'sG', 'yy', 'yn', 't', 'b', 'gn', 'go', "'", 'ymd'
  ],
